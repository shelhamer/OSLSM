...
def test_net(loader, model_path, weights_path, output_dir, test_iters, gpu, test_cats_indicator=range(20), save_images=True):
    ...
    for ti in range(test_iters):
        ...
        # TODO(shelhamer) just save the set
        ...
        if save_images:
            ...
            gt = Image.fromarray(deploy_info['second_mask_orig'].astype(np.uint8))
            gt.save(os.path.join(final_mask_dir, 'class%d-qry-gt-%05d.png' % (class_ind + 1, ti)))
            gt = Image.fromarray(deploy_info['first_mask_orig'][0].astype(np.uint8))
            gt.save(os.path.join(final_mask_dir, 'class%d-sup-gt-%05d.png' % (class_ind + 1, ti)))
        # # Debug: Show Stuff
